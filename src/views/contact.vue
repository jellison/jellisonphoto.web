<template>
  <div ref="container">
    ...
    <div class="container main">
          ...
          <div class="column">
            <div class="column-content">
              <p>
                ...
                <a href="mailto:info@jellisonphoto.com">info@jellisonphoto.com</a>
      ...
      <div v-else class="is-flex has-justified-center mt-4 mb-4 center-logo">
        <router-link to="/">
      ...
      <form class="main" v-if="!mailingList">
              ...
              <div class="control">
                <div class="select">
                  <select v-model="fields.sessionType">
                    <option :key="sessionType" v-for="sessionType of sessionTypes">
                      {{ sessionType }}
                    </option>
                  </select>
                </div>
        ...
        <div class="columns" v-if="isWedding">
          <div class="column">
            <div class="field">
              ...
              <flat-select :values="['Basic', 'Silver', 'Gold', 'Platinum']" v-model="fields.package"></flat-select>
            </div>
          </div>
          <div class="column">
            <div class="field">
        ...
        <div class="field has-text-right">
          <a v-if="formOnly" class="button is-outlined" @click.prevent="back">Back</a>
...
export default {
  ...
  data() {
    ...
    var sessionTypes = ['Regular Session', 'Senior Session', 'Newborn Session', 'Studio Session', 'Wedding'];

    if (sessionType && !sessionTypes.includes(sessionType)) sessionTypes.push(sessionType);
  ...
  methods: {
    ...
    submitEmail(evt) {
      axios
        ...
        .then((res) => {
          $(evt.target).replaceWith('<div class="notification is-info mt-20 has-text-centered">Message sent successfully!</div>');
        })
        .catch((err) => {
    ...
    joinList(evt) {
      axios
        .post(`${baseUrl}/emails/lists/${this.mailingList.id}`, {
          name: this.fields.name,
          email: this.fields.email
        })
        .then((res) => {
          $(evt.target).replaceWith('<div class="notification is-info mt-20 has-text-centered">Thanks for joining the mailing list!</div>');
        })
        .catch((err) => {
          $(evt.target).replaceWith(
            '<div class="notification is-danger mt-20 has-text-centered">Something went wrong :( Please email us directly at <a href="mailto:info@jellisonphoto.com">info@jellisonphoto.com</a></div>'
